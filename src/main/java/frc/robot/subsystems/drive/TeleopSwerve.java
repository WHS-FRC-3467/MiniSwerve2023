...
public class TeleopSwerve extends CommandBase {
    ...
    public TeleopSwerve(DriveSubsystem swerve, DoubleSupplier xSup, DoubleSupplier ySup,
        ...
        m_speedChooser.setDefaultOption("100%", 1.0);
    ...
    public void initialize() {
        m_thetaController = new PIDController(SwerveConstants.GAINS_ANGLE_SNAP.kP, SwerveConstants.GAINS_ANGLE_SNAP.kI,
                SwerveConstants.GAINS_ANGLE_SNAP.kD);
    ...
    public void execute() {
        ...
        if (rotateWithButton) {
            ...
            rotationVal = m_thetaController.calculate(
                    (MathUtil.inputModulus(m_Swerve.getPose().getRotation().getDegrees(), -180, 180)),
                    m_thetaController.getSetpoint());
            rotationVal = MathUtil.clamp(rotationVal, -SwerveConstants.MAX_ANGULAR_VELOCITY * 0.075,
                    SwerveConstants.MAX_ANGULAR_VELOCITY * 0.075);
            ...
            rotationVal = (MathUtil.applyDeadband(rotationSup.getAsDouble() * m_speedChooser.getSelected(),
                    SwerveConstants.DRIVE_DEADBAND)) * 0.75;
